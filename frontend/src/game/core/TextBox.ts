...
export default class TextBox extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Scene, size: { width: number, height: number }, position: Phaser.Math.Vector2) {
    ...
    this.background.fillRoundedRect(0, 0, this.size.width, this.size.height);

    this.text = new Phaser.GameObjects.Text(this.scene, this.size.width * 0.5, this.size.height * 0.5, '', { ...Text.Dialog1, wordWrap: { width: this.size.width } }).setOrigin(0.5).setDepth(100);
    this.setDepth(1010);
